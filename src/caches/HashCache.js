...
module.exports = class HashCache extends Cache {
  constructor(
    ...
    this._revive = (field, value) => {
      ...
      assert(
        state.length === 2,
        'revive must return an array with length of 2 (state key and value)'
      )

      const [stateKey, stateValue] = state
      assert(
        typeof stateKey === 'string',
        'revive return an array with a string at position 0 (state key)'
      )
  ...
  initialize(values) {
    ...
    return this._initialize([
      [
        'hmset',
        this._key,
        ...flatMap(values, (value, field) => this._composeArguments(field, value)),
      ],
    ])
  }
